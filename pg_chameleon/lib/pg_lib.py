...
class pg_engine(object):
    ...
    def __quote_identifier(self, identifier):
        """
            The method quotes an SQL identifier using the active PostgreSQL connection.
        """
        return sql.Identifier(identifier).as_string(self.pgsql_conn)

    def __quote_literal(self, value):
        """
    ...
    def __build_where_clause(self, pkey_columns, event_before, event_after):
        """
            The method builds a primary-key predicate for replay statement previews.
        """
        where_parts = []
        for pkey_column in pkey_columns or []:
            if event_before and pkey_column in event_before:
                pkey_value = event_before[pkey_column]
            else:
                pkey_value = (event_after or {}).get(pkey_column)
            where_parts.append("%s=%s" % (self.__quote_identifier(pkey_column), self.__quote_literal(pkey_value)))
        return " AND ".join(where_parts)

    def __build_replay_statement_preview(self, replay_event):
        """
            The method builds a client-side preview of the SQL generated by
            fn_replay_mysql for the first queued event.
        """
        event_type = replay_event["enm_binlog_event"]
        schema = self.__quote_identifier(replay_event["v_schema_name"])
        table = self.__quote_identifier(replay_event["v_table_name"])
        event_after = replay_event["jsb_event_after"] or {}
        event_before = replay_event["jsb_event_before"] or {}
        pkey_columns = replay_event["v_table_pkey"] or []

        if event_type == "ddl":
            return replay_event["t_query"]
        if event_type == "insert":
            columns = ",".join([self.__quote_identifier(column) for column in event_after.keys()])
            values = ",".join([self.__quote_literal(value) for value in event_after.values()])
            return "INSERT INTO %s.%s (%s) VALUES (%s);" % (schema, table, columns, values)
        if event_type == "update":
            set_clause = ",".join(
                [
                    "%s=%s" % (self.__quote_identifier(column), self.__quote_literal(value))
                    for column, value in event_after.items()
                ]
            )
            where_clause = self.__build_where_clause(pkey_columns, event_before, event_after)
            return "UPDATE %s.%s SET %s WHERE %s;" % (schema, table, set_clause, where_clause)
        if event_type == "delete":
            where_clause = self.__build_where_clause(pkey_columns, event_before, event_after)
            return "DELETE FROM %s.%s WHERE %s;" % (schema, table, where_clause)
        return None

    def get_replay_statement_preview(self, id_batch, id_event):
        """
            The method returns the SQL preview for the first queued replay event.
            This is used for diagnostics because PostgreSQL notices raised by
            fn_replay_mysql are only visible to the client after the function returns.
        """
        sql_log_table = """
            SELECT
                v_log_table
            ...
            ;
        """
        self.pgsql_cur.execute(sql_log_table, (id_batch, ))
        log_table = self.pgsql_cur.fetchone()
        if not log_table:
            return None
        sql_event = sql.SQL("""
            SELECT
                log.i_id_event,
                log.enm_binlog_event::text AS enm_binlog_event,
                log.v_schema_name,
                log.v_table_name,
                log.t_query,
                log.jsb_event_after,
            ...
            FROM
                ...
                INNER JOIN sch_chameleon.t_replica_tables tab
                    ON tab.i_id_source=%s
                    AND tab.v_schema_name=log.v_schema_name
                    AND tab.v_table_name=log.v_table_name
                    AND tab.b_replica_enabled
            WHERE
                    log.i_id_batch=%s
                AND log.i_id_event=%s
            LIMIT 1
            ;
        """).format(sql.Identifier(log_table[0]))
        self.pgsql_cur.execute(sql_event, (self.i_id_source, id_batch, id_event, ))
        replay_event = self.pgsql_cur.fetchone()
        if not replay_event:
            return None
        replay_event = dict(zip([column[0] for column in self.pgsql_cur.description], replay_event))
        return self.__build_replay_statement_preview(replay_event)
    ...
    def ensure_log_replica_indexes(self):
        """
            The method ensures the source log tables have the indexes needed by
            the replay function. Existing catalogues may have only the batch
            index created by fn_refresh_parts().
        """
        self.set_source_id()
        sql_log_tables = """
            SELECT
        ...
        for log_table in log_tables:
            ...
            index_suffix = hashlib.md5(log_table_name.encode("utf-8")).hexdigest()[:12]
            batch_event_index = "idx_log_be_%s" % index_suffix
            batch_table_index = "idx_log_bt_%s" % index_suffix
            self.logger.debug("Ensuring replay indexes on log table %s" % log_table_name)
            sql_batch_event = sql.SQL("""
                CREATE INDEX IF NOT EXISTS {}
                ON sch_chameleon.{} (i_id_batch, i_id_event)
                ;
            """).format(
                sql.Identifier(batch_event_index),
                sql.Identifier(log_table_name)
            )
            sql_batch_table = sql.SQL("""
                CREATE INDEX IF NOT EXISTS {}
                ON sch_chameleon.{} (i_id_batch, v_schema_name, v_table_name)
                ;
            """).format(
                sql.Identifier(batch_table_index),
                sql.Identifier(log_table_name)
            )
            self.pgsql_cur.execute(sql_batch_event)
            self.pgsql_cur.execute(sql_batch_table)
    ...
    def restore_stored_idx_cons(self):
        """
            The method recreates any previously collected primary keys, unique
            constraints and indexes for the current source.

            This is a safety net for keep_existing_schema runs where an initial
            copy failed after dropping target indexes. Without this, replay can
            get stuck doing row updates against unindexed target tables.
        """
        self.set_source_id()
        sql_tables = """
            WITH stored_tables AS
            (
                    SELECT v_schema_name, v_table_name FROM sch_chameleon.t_indexes
                UNION
                    SELECT v_schema_name, v_table_name FROM sch_chameleon.t_pkeys
                UNION
                    SELECT v_schema_name, v_table_name FROM sch_chameleon.t_ukeys
            )
            SELECT
                st.v_schema_name,
                st.v_table_name
            FROM
                ...
                INNER JOIN sch_chameleon.t_replica_tables tab
                    ON tab.i_id_source=%s
                    AND tab.v_schema_name=st.v_schema_name
                    AND tab.v_table_name=st.v_table_name
            ORDER BY
                st.v_schema_name,
                st.v_table_name
            ;
        """
        self.pgsql_cur.execute(sql_tables, (self.i_id_source, ))
        stored_tables = self.pgsql_cur.fetchall()
        if stored_tables:
            self.logger.warning(
                "Restoring stored target indexes and constraints for %s table(s) before replay."
                % len(stored_tables)
            )
        for stored_table in stored_tables:
            schema = stored_table[0]
            table = stored_table[1]
            self.logger.info("Restoring stored target indexes and constraints for %s.%s" % (schema, table))
            self.create_idx_cons(schema, table)
